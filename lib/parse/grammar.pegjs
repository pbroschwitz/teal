...
Mixin
  = '@mixin' __ decl:Block {
    return pos({ type: 'mixin', declarations: decl })
  }

Include
  = '@include' __ p:Path __ '{' __ decl:CssContent* '}' {
    return pos({ type: 'include', path: p, declarations: decl })
  }

Defaults
  = '@defaults' __ '{' __ decl:DefaultsContent* '}' {
    return pos({ type: 'defaults', declarations: decl })
  }

DefaultsContent
  = d:(Property / Attribute) __ { return d }
...
CssFunction
  = name:FunctionName '(' __ args:CssArgumentList __ ')' {
...
CssArgumentList
  = n:CssArgument? m:(__ ',' __ CssArgument)* { return list(n, m, 3) }
...
Animation
  = '@animation' __ v:AnimationValue __ '{' __ f:Keyframe* __ '}' {
    return pos({
      type: 'animation',
      value: v.trim(),
      keyframes: { type: 'keyframes', frames: f }
    })
  }
...
Selector "selector"
  = $(':' [a-z0-9():-]i+)
  / $('[' (!']')* ']')
  / '~' n:([1-9][0-9]*)? s:$([a-z0-9():-]i+) { return { adjacentSibling: parseInt(n||1), selector: s } }
  / '.' s:$([a-z]i[a-z0-9_]i*)+ { return { state: s } }
  / p:('^'+) s:Selector { return { parent: p.length, selector: s } }
...
If
  = '@if' __ n:'not'? __ c:Exp _ con:DirectiveContent alt:Else? {
...
BinaryExpression
  = left:UnaryExpression S+ op:BinaryOperator S+ right:BinaryExpression {
...
StringLiteral "string"
  = '"' chars:DoubleStringCharacter* '"' { return interpolate(chars.join('')) }
