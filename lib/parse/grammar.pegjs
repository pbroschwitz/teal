...
Start
  = __ d:Doctype? __ v:Assignment* __ r:Root? __ EOF {
    return pos({ type: 'root', doctype: d, variables: v, root: r })
  }
...
For
  = '@for' _ c:Exp _ body:DirectiveContent {
...
Selector
  = $(('::' / ':') [a-z0-9()-]i+)
  / $('[' (!']')* ']')
  / 'is-' ([a-z]i[a-z0-9_-]*)+  { return { state: text() }}
...
CssFunction
  = name:CssIdentifier '(' args:ArgumentList ')' {
    return options.expand({ type: 'function', name: name, arguments: args })
  }
