...
function Teal(opts) {
  ...
  require.extensions['.tl'] = function(module, filename) {
    module.exports = teal.require(filename)
  }

  this.on('mount', function(app) {
    this.opts.root = app.get('views')
    app.engine('tl', this.engine)
  ...
  this.watcher.on('steady', function() {
    ...
    try {
      for (var i = 0; i < teal.queue.length; i++) {
        ...
        var res = teal.expose({ file: file }) //if dev
        var ast = teal.parse(file)
        if (ast.root) {
          teal.enqueue(ast.deps)
          teal.emit('file', file, ast, res.url)
        }
      }
    }
    catch (err) {
