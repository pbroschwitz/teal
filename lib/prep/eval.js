...
module.exports = visitor({
  ...
  _context: {
    visitChildren: function(node) {
      ...
      if (node.mixin) {
        ...
        // eval attributes and set them as vars
        this.visit(node.attributes, { vars: vars, set: true })

        // expose nested nodes as $children
        vars.set('children', this.visit(node.children, { eval: true }))
  ...
  declaration: function(node) {
    var d = this.mapObject(node, { eval: true })
  ...
  function: function(node) {
    ...
    var m = this.teal.macros[node.name] //TODO Use teal.css.functions instead
