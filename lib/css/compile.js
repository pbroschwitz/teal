...
var cssVisitor = visitor({
  ...
  _context: {
    ...
    prev: function(i, sel) {
      ...
      var rel = local(prev.selectors[0], this.parent.selectors[0])

      var c = /^:/.test(sel) ? '' : ' '
      var sib = comb(rel, sel, c)

      return comb(sib, this.parent.selectors, ' ~ ')
    },
    ...
    expandStyleSelectors: function(node) {
      ...
      node.selectors.forEach(function(s) {
        if (s.state) {
          ...
          // +:checked { ... }
          addTo(sel, comb(s.adjacentSibling, current, ' + '))
        }
        else {
          var c
