...
module.exports = visitor({
  ...
  call: function(node) {
    ...
    return callee.apply(this.ctx, args)
  },
  ...
  element: function(node) {
    ...
    node.declarations = node.declarations.filter(function(d) {
      return !~node.attributes.indexOf(d)
        && !~node.children.indexOf(d)
    })
    if (node.name.path) {
      node.path = this.ctx.resolve(node.name.path)
    }
...
function pluckContent(node) {
  return flatten(node.declarations||[]).filter(function(n) {
    if (!n) console.log(node)
    return !n.type || n.isContent
  })
}
