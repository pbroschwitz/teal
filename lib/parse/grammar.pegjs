...
Element
  = name:(Exp / PathLiteral / Identifier) cls:ClassName? __ decl:Declarations {
...
Selector "selector"
  ...
  / $('*' [a-z0-9():-]i*)
  / $('[' (!']' .)+ ']')
  / '~' n:([1-9][0-9]*)? s:$([a-z0-9():-]i+) { return { sibling: parseFloat(n||1), selector: s } }
  / '+' s:$(Selector) { return { adjacentSibling: s } }
...
CallExpression "call"
  = first:(
      callee:(MemberExpression / PathLiteral / Identifier) __ args:Arguments {
