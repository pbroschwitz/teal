...
module.exports = function(root, opts) {
  ...
  function withDependencies(files) {
    var all = files.slice()
    files.forEach(function(f) {
      addUnique(all, deps[f])
    })
    return all
  }

  function getTansitiveDeps(f) {
    var dest = []
    function addDeps(f) {
      dest.push(f)
      var files = deps[f]
      if (files) files.forEach(function(d) {
        if (!~dest.indexOf(d)) addDeps(d)
      })
    }
    addDeps(f)
    return dest
  ...
  function context(file) {
    ...
    return xtend(opts, {
      ...
      getDependants: function(f) {
        return dependants[f || file]
      },

      getDependencies: function(f) {
        return getTansitiveDeps(f || file)
      },

      dependsRecursive: function(f) {
        if (f == file) return
        this.depends(f)
        if (deps[f]) deps[f].forEach(this.dependsRecursive, this)
      },
