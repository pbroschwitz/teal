  ...
  return function(on, visit) {
    ...
    on.root = function(node) {
      ...
      return join(
        ...
        f('module.exports=exports=function(data){\n%s\n}', join(
          f('var scope=require("%s")(arguments, this)', __dirname + '/scope'),
    ...
    on.each = function(node) {
      return f('_.each(%s, %j, scope, function(scope) { return %s })',
        visit(node.expression), node.variable.name, visit(node.body)
      )
    }
    ...
    on.concat = function(node) {
      return visit(node.left) + '+' + visit(node.right)
    }

    on.default = function(node) {
      return f('(%s||%s)', visit(node.expression), visit(node.default))
    }
