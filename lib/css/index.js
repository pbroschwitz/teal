...
CSS.prototype.process = function(mods, bases) {
  var rules = []
  ...
  mods.forEach(function(ast) {
    ...
    visitor.visit(ast, function(on, visit) {
      ...
      on.element = function(node) {
        if (node.class || node.module)
          node.class = [node.class, node.module, node.module + '-' + node.tag].join(' ').trim()
